default_questions = [
  ...
  },
  {
    "question":
      ...
      "total margin ,"
      "percentage margin "
      "for the quarter that ends on {DATE} for city?",
    "query": """
      SELECT
        ds.city,
        SUM(fs.sales_amount) AS total_sales,
  ...
  },
  {
    "question":
      ...
      "items_per_transaction "
      "for the quarter that ends on {DATE} for all stores and in which city is each store located?",
    "query": """
      SELECT
        ds.store_id,
        ds.store_name,
  ...
  },
  {
    "question":
      ...
      "percentage margin, "
      "total units sold "
      "for the quarter that ends on {DATE} for all categories ",
    "query": """
      SELECT
        dc.category_id,
        dc.category_name,
  ...
  },
  {
    ...
    "query": """
      ...
      SELECT 
        COALESCE(current_quarter.vendor_id, 0) AS vendor_id, 
        COALESCE(current_quarter.vendor_name, '') AS vendor_name, 
        COALESCE(current_quarter.total_sales_by_vendor, 0) AS current_quarter_total_sales_by_vendor, 
        COALESCE(current_quarter.class_abc, 0) AS current_class_abc, 
        COALESCE(current_quarter.class_xyz, 0) AS current_class_xyz,
        COALESCE(last_quarter.total_sales_by_vendor, 0) AS last_quarter_total_sales_by_vendor, 
        COALESCE(last_quarter.class_abc, 0) AS last_class_abc, 
        COALESCE(last_quarter.class_xyz, 0) AS last_class_xyz,
        COALESCE((current_quarter.class_abc - last_quarter.class_abc), 0) AS class_abc_change, 
        COALESCE((current_quarter.class_xyz - last_quarter.class_xyz), 0) AS class_xyz_change
      FROM current_quarter
      JOIN last_quarter ON current_quarter.vendor_id = last_quarter.vendor_id
      WHERE (current_quarter.class_abc - last_quarter.class_abc) != 0 OR (current_quarter.class_xyz - last_quarter.class_xyz) != 0
      ORDER BY ABS(current_quarter.class_abc - last_quarter.class_abc) DESC, (current_quarter.class_abc - last_quarter.class_abc) DESC, ABS(current_quarter.class_xyz - last_quarter.class_xyz) DESC, (current_quarter.class_xyz - last_quarter.class_xyz) DESC    
      LIMIT 20
      """
    ...
    "query": """
      ...
      (
        ...
        SELECT 
          ...
          (SUM(s.confidence_interval_lower_bound) - SUM(c.confidence_interval_lower_bound)) / NULLIF(SUM(s.confidence_interval_lower_bound), 0) AS confidence_interval_lower_bound, 
          (SUM(s.confidence_interval_upper_bound) - SUM(c.confidence_interval_upper_bound)) / NULLIF(SUM(s.confidence_interval_upper_bound), 0) AS confidence_interval_upper_bound 
        FROM sales s, costs c
      ), 
      actual AS 
      (
  ...
  },
  {
    ...
    "query": """
      SELECT 
        ...
        COALESCE(SUM(confidence_interval_upper_bound), 0) AS upper_bound 
      FROM 
        ML.FORECAST(MODEL modelo_arima_dia_cost, STRUCT(100 AS horizon)) 
        WHERE 
            EXTRACT(QUARTER FROM forecast_timestamp) = EXTRACT(QUARTER FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY)) AND
            EXTRACT(YEAR FROM forecast_timestamp) = EXTRACT(YEAR FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY))
    """
  },
  {
    ...
    "query": """
      ...
      (
        SELECT * 
        FROM ML.FORECAST(MODEL modelo_arima_dia, STRUCT(100 AS horizon, 0.99 AS confidence_level))
        WHERE 
            EXTRACT(QUARTER FROM forecast_timestamp) = EXTRACT(QUARTER FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY)) AND
            EXTRACT(YEAR FROM forecast_timestamp) = EXTRACT(YEAR FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY))
      ), 
      ...
      (
        SELECT * 
        FROM ML.FORECAST(MODEL modelo_arima_dia_cost, STRUCT(100 AS horizon, 0.99 AS confidence_level))
        WHERE 
            EXTRACT(QUARTER FROM forecast_timestamp) = EXTRACT(QUARTER FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY)) AND
            EXTRACT(YEAR FROM forecast_timestamp) = EXTRACT(YEAR FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY))
      ) 
  ...
  },
  {
    "question":
      ...
      "lower bound, "
      "upper bound "
      "for the next quarter for category?",
    "query": """
      SELECT 
        ...
        COALESCE(SUM(confidence_interval_lower_bound), 0) AS lower_bound, 
        COALESCE(SUM(confidence_interval_upper_bound), 0) AS upper_bound 
      FROM ML.FORECAST(MODEL modelo_arima_dia_categories, STRUCT(100 AS horizon)) 
      WHERE 
            EXTRACT(QUARTER FROM forecast_timestamp) = EXTRACT(QUARTER FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY)) AND
            EXTRACT(YEAR FROM forecast_timestamp) = EXTRACT(YEAR FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY))
  ...
  },
  {
    "question":
      ...
      "lower bound, "
      "upper bound "
      "for the next quarter for store?",
    "query": """
      SELECT 
        ...
        COALESCE(SUM(confidence_interval_lower_bound), 0) AS lower_bound, 
        COALESCE(SUM(confidence_interval_upper_bound), 0) AS upper_bound 
      FROM ML.FORECAST(MODEL modelo_arima_dia_stores, STRUCT(100 AS horizon)) 
      WHERE 
            EXTRACT(QUARTER FROM forecast_timestamp) = EXTRACT(QUARTER FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY)) AND
            EXTRACT(YEAR FROM forecast_timestamp) = EXTRACT(YEAR FROM DATE_ADD(DATE('{DATE}'), INTERVAL 1 DAY))
      GROUP BY store_name
