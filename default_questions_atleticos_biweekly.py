default_questions = [
  ...
  },
  {
    "question":
      ...
      "items_per_transaction "
      "for the fortnight that ends on {DATE} for all stores?",
    "query": """
      SELECT
        ds.store_id,
        ds.store_name,
    ...
    "query": """
      ...
      (
        ...
        SELECT 
          SUM(s.forecast_value) AS total_sales, 
          SUM(c.forecast_value) AS total_cost,  
          (SUM(s.forecast_value) / NULLIF(SUM(c.forecast_value), 0) AS margin, 
          (SUM(s.prediction_interval_lower_bound) / NULLIF(SUM(c.prediction_interval_lower_bound), 0) AS prediction_interval_lower_bound, 
          (SUM(s.prediction_interval_upper_bound) / NULLIF(SUM(c.prediction_interval_upper_bound), 0) AS prediction_interval_upper_bound 
        FROM sales s, costs c
      ), 
      actual AS 
      (
  ...
  },
  {
    ...
    "query": """
      ...
      SELECT 
        (COALESCE(SUM(s.forecast_value), 0) / NULLIF(SUM(c.forecast_value), 0)) AS margin, 
        (COALESCE(SUM(s.prediction_interval_lower_bound), 0) / NULLIF(SUM(c.prediction_interval_lower_bound), 0)) AS prediction_interval_lower_bound, 
        (COALESCE(SUM(s.prediction_interval_upper_bound), 0) / NULLIF(SUM(c.prediction_interval_upper_bound), 0)) AS prediction_interval_upper_bound 
      FROM sales s, costs c
    """
  },
  {
