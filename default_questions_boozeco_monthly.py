default_questions = [
...

  {
    ...
    "query": """
      ...
      (
        ...
        WHERE 
          EXTRACT(DATE FROM sn.snapshot_date) = DATE_SUB(DATE('{DATE}'), INTERVAL EXTRACT(DAY FROM DATE('{DATE}')) DAY)
        GROUP BY 
          st.store_id,
          st.store_name
      ), 
      ...
      (
        ...
        WHERE 
          EXTRACT(MONTH FROM m.movement_date) = EXTRACT(MONTH FROM DATE('{DATE}')) AND
  ...
  },
  {
    "question":
      ...
      "last sale, "
      "days without sales "
      "for the month that ends on {DATE} for the top {top_n_high_performance_items} items with more days without sales.",
    "query": """
      SELECT 
        i.item_id,
        i.item_name,
  ...
  },
  {
    "question":
      ...
      "last sale, "
      "days without sales "
      "for the month that ends on {DATE} for the top {top_n_high_performance_categories} categories with more days without sales.",
    "query": """
      SELECT 
        c.category_id,
        c.category_name,
